...
fn build_plan_rejects_params_violating_config_schema() {
    ...
    let result = engine.build_plan(&ctx, &verified, &bad_params, &ctx.execution_hints, None, ryeos_engine::isolation::IsolationFilesystemAuthorityCeiling::NodePolicy);
...
fn build_plan_accepts_params_conforming_to_config_schema() {
    ...
    let result = engine.build_plan(&ctx, &verified, &good_params, &ctx.execution_hints, None, ryeos_engine::isolation::IsolationFilesystemAuthorityCeiling::NodePolicy);
