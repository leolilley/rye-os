...
const RUNTIME_OPERATOR_SCHEMA_EPOCH: u32 = 22;
const _: () = assert!(
...
impl RuntimeDb {
    ...
    /// Select the latest owner-authored route coordinate for one placement.
    ///
    /// This operational value is not command or completion authority. Callers
    /// must pass the returned coordinate through the placement-root fact
    /// verifier before using it. Keeping this selector here avoids either a
    /// second completion ledger or an unbounded API-layer scan.
    pub fn latest_dedicated_session_route_command_sequence(
        &self,
        placement_thread_id: &str,
    ) -> Result<Option<u64>> {
        validate_bounded_runtime_text("command placement thread id", placement_thread_id, 256)?;
        self.conn
            ...
            .map(u64::try_from)
            .transpose()
            .context("latest dedicated route-command sequence overflow")
    }
