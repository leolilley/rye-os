...
//! chain, command-completion, candidate-capture, and validation authorities.
...
const OPERATION_SCHEMA: &str = "ryeos.remote_hosted_worker_result_pull_operation.v1";
const INTENT_SCHEMA: &str = "ryeos.remote_hosted_worker_result_pull_intent.v1";
const RESPONSE_SCHEMA: &str = "ryeos.remote_hosted_worker_result_pull_result.v1";
...
async fn query_candidate_result(
    ...
    let response: HostedCandidateResultResponse =
        serde_json::from_value(value).context("decode hosted candidate testimony")?;
