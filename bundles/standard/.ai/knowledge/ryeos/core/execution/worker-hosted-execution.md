...
source-side command for returning a validated retained project candidate; it
...
origin-bound configured owner and reconstructs current chain/placement,
latest settled route command and turn-completion fence, admitted project/base,
candidate capture, and closure/base validation from existing authority. Its
node-signed attestation is evidence, not a second candidate ledger and not
publication authority. The source retains the exact attestation and route in
...
does not advance either project HEAD and leaves the target disposition
`retained`; publication or discard remains a separate explicit owner action.
